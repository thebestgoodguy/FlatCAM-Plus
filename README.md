...
forked from FlatCAM Plus (c) 2019 - by Marius Stanciu
...
*   **Windows 11 Full Compatibility:** Optimized core logic for Windows operating systems, resolving stability issues found in previous versions.
*   **Threading Safety Fixes:** Resolved critical "RuntimeError: wrapped C/C++ object has been deleted" crashes by implementing thread-safe UI access in the background plotting engine.
*   **Enhanced Gerber Rendering:** Improved geometry union stability and buffering precision (1e-6) to ensure PCB traces are correctly visualized without disappearing.
*   **Robust Plugin Loading:** Implemented safe import mechanisms to prevent application-wide crashes due to missing third-party dependencies (like GDAL/Rasterio).
*   **Modernized Environment:** Updated for Python 3.11+ and PyQt6, ensuring long-term maintainability.
