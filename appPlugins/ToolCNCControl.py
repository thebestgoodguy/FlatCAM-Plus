    ...
    def __init__(self, text="", color="#31b0d5", hover="#269abc", text_color="white", parent=None):
        ...
        self.setStyleSheet(f"""
            QPushButton {{
                ...
                border-radius: 3px; padding: 5px 12px; font-weight: bold; font-size: 9pt; min-height: 30px;
            }}
...
class ToolCNCControl(AppTool):
    ...
    def on_toggle_laser(self):
        if "M3" in self.ui.macro_laser.text(): self.send_command("M5"); self.ui.macro_laser.setText("LASER ON")
        else: self.send_command("M3 S100"); self.ui.macro_laser.setText("LASER OFF")
    ...
    def update_status_display(self, data):
        ...
        self.ui.state_indicator.setStyleSheet(f"background-color: {colors.get(state, '#aaa')}; border-radius: 4px;")
        if "WPos" in data:
    ...
    def stream_worker(self):
        while self.is_streaming and self.current_line_idx < len(self.gcode_lines):
            ...
            self.ok_received.wait(timeout=4.0); self.current_line_idx += 1
...
class CNCControlUI:
    ...
    def __init__(self, layout, app):
        ...
        self.main_lay = QtWidgets.QVBoxLayout(container); self.main_lay.setContentsMargins(5, 5, 5, 5); self.main_lay.setSpacing(5)

        # --- HEADER (1:1 ESP3D) ---
        header = QtWidgets.QFrame()
        header.setFixedHeight(50); header.setStyleSheet(f"background-color: {self.primary}; color: white; border-bottom: 2px solid rgba(0,0,0,0.1);")
        ...
        self.state_label = FCLabel("OFFLINE", bold=True, size=9, color="white")
        h_lay.addWidget(self.state_indicator); h_lay.addWidget(self.state_label); h_lay.addSpacing(20)
        self.com_port = FCComboBox(); self.com_port.setMinimumWidth(120); self.com_port.setStyleSheet("color: #333; background: white; padding: 4px;")
        self.com_refresh = QtWidgets.QPushButton(); self.com_refresh.setFixedSize(26, 26); self.com_refresh.setIcon(QtGui.QIcon(self.app.resource_location + '/reload32.png'))
        self.com_refresh.setStyleSheet("background: rgba(255,255,255,0.1); border: none; border-radius: 4px;")
        self.connect_btn = FluidStyleButton(_("CONNECT"), "#ffffff", "#f1f1f1", self.primary)
        ...
        # --- GRID LAYOUT (NO SPACES) ---
        grid = QtWidgets.QGridLayout(); grid.setSpacing(5); self.main_lay.addLayout(grid)
        ...
        def add_dro(axis, r, color):
            l = FCLabel(axis, bold=True, size=12, color=color); l.setFixedWidth(25)
            z = FluidStyleButton("0", self.primary); z.setFixedSize(28, 28)
            v = FCLabel("0.000", bold=True, size=28); v.setStyleSheet("font-family: 'Consolas'; color: #333;")
        ...
        # JOG CARD
        jog_card = self.create_card(_("CONTROL & SD"))
        ...
        jg = QtWidgets.QGridLayout(); jg.setSpacing(6); jg.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        sd_lay = QtWidgets.QHBoxLayout(); self.sd_combo = FCComboBox(); self.sd_list_btn = FluidStyleButton("⟳", "#2d3436"); self.run_sd_btn = FluidStyleButton("RUN", "#00b894")
        ...
        # TOOLS CARD
        tools_card = self.create_card(_("OVERRIDES & SYSTEM"))
        ...
        ov_grid.addWidget(FCLabel(_("FEED"), bold=True, size=8), 0, 0); ov_grid.addWidget(self.feed_minus, 0, 1); ov_grid.addWidget(self.feed_reset, 0, 2); ov_grid.addWidget(self.feed_plus, 0, 3)
        ov_grid.addWidget(FCLabel(_("SPINDLE"), bold=True, size=8), 1, 0); ov_grid.addWidget(self.spindle_minus, 1, 1); ov_grid.addWidget(self.spindle_reset, 1, 2); ov_grid.addWidget(self.spindle_plus, 1, 3)
        
        btn_lay = QtWidgets.QHBoxLayout(); self.macro_probe = FluidStyleButton("PROBE Z", "#3498db"); self.macro_laser = FluidStyleButton("LASER ON", "#e17055")
        btn_lay.addWidget(self.macro_probe); btn_lay.addWidget(self.macro_laser); tools_card.layout().addLayout(btn_lay)
        sys_lay = QtWidgets.QHBoxLayout(); self.cfg_dump = FluidStyleButton("CFG", "#2d3436"); self.info_btn = FluidStyleButton("INFO", "#2d3436"); self.reset_btn = FluidStyleButton("RST", "#00b894"); self.estop_btn = FluidStyleButton("STOP", "#d63031")
        ...
        self.console = FCTextArea(); self.console.setReadOnly(True); self.console.setStyleSheet("background: #1e1e1e; color: #abb2bf; font-family: 'Consolas'; font-size: 10pt; border: none; padding: 5px;")
        ...
        self.play_btn = FluidStyleButton("RUN JOB", "#00b894"); self.pause_btn = FluidStyleButton("PAUSE", "#fdcb6e"); self.stop_btn = FluidStyleButton("STOP", "#d63031")
        ...
        prog_lay = QtWidgets.QHBoxLayout(); self.object_combo = FCComboBox(); self.progress = QtWidgets.QProgressBar(); self.progress.setFixedHeight(8)
        prog_lay.addWidget(FCLabel(_("Active:"), size=8)); prog_lay.addWidget(self.object_combo, 1); prog_lay.addWidget(self.progress, 2)
        term_card.layout().addLayout(prog_lay)

    def create_card(self, title):
        card = QtWidgets.QFrame()
        card.setStyleSheet(f"background: #ffffff; border-radius: 6px; border: 1px solid #dfe6e9;")
        lay = QtWidgets.QVBoxLayout(card); lay.setContentsMargins(12, 12, 12, 12); lay.setSpacing(8)
        t_lbl = FCLabel(f"<b>{title}</b>", size=10, color=self.primary); lay.addWidget(t_lbl)
    ...
    def set_connected(self, connected):
        ...
        btns = [self.play_btn, self.pause_btn, self.stop_btn, self.jog_up, self.jog_down, self.jog_left, self.jog_right, self.jog_z_up, self.jog_z_down, 
                ...
                self.macro_probe, self.macro_laser]
    ...
    def append_console(self, text, type):
        ...
        timestamp = f'<span style="color: #5c6370;">[{time.strftime("%H:%M:%S")}]</span>'
        self.console.appendHtml(f'{timestamp} <span style="color: {color};">{text}</span>')
