...
class CNCJobObject(FlatCAMObj, CNCjob):
    ...
    def __init__(self, name, app, units="in", kind="generic", z_move=0.1,
        ...
        self.obj_options.update({
            ...
            "tools_al_travel_z": self.app.options.get("tools_al_travel_z", 5.0),
            "tools_al_probe_depth": self.app.options.get("tools_al_probe_depth", -1.0),
            "tools_al_probe_fr": self.app.options.get("tools_al_probe_fr", 120.0),
            "tools_al_controller": self.app.options.get("tools_al_controller", "GRBL"),
            "tools_al_method": self.app.options.get("tools_al_method", "grid"),
            "tools_al_mode": self.app.options.get("tools_al_mode", "manual"),
            "tools_al_rows": self.app.options.get("tools_al_rows", 4),
            "tools_al_columns": self.app.options.get("tools_al_columns", 4),
            "tools_al_grbl_jog_step": self.app.options.get("tools_al_grbl_jog_step", 1.0),
            "tools_al_grbl_jog_fr": self.app.options.get("tools_al_grbl_jog_fr", 300.0),
        })
    ...
    def set_ui(self, ui):
        ...
        levelling_tool = getattr(self.app, 'levelling_tool', None)
        if callable(getattr(levelling_tool, 'run', None)):
            self.ui.autolevel_button.clicked.connect(lambda: levelling_tool.run(toggle=True))
        else:
            self.ui.autolevel_button.hide()
            self.ui.tool_lbl.hide()
    ...
    def gcode_header(self, comment_start_symbol=None, comment_stop_symbol=None):
        ...
        if marlin is True:
            gcode += ';Marlin(Repetier) G-code generated by FlatCAM Plus v%s - Version Date:    %s\n' % \
            ...
            gcode += self.machine_profile_gcode_header(style="semicolon")
        elif hpgl is True:
            gcode += 'CO "HPGL code generated by FlatCAM Plus v%s - Version Date:    %s' % \
                     (str(self.app.version), str(self.app.version_date)) + '";\n'
            ...
            gcode += self.machine_profile_gcode_header(style="hpgl")
        elif probe_pp is True:
            gcode += '(G-code generated by FlatCAM Plus v%s - Version Date: %s)\n' % \
                     (str(self.app.version), str(self.app.version_date)) + '\n'
            ...
            gcode += '(Units: ' + self.units.upper() + ')\n' + "\n"
            gcode += '(Created on ' + time_str + ')\n' + '\n'
            gcode += self.machine_profile_gcode_header()
        elif nccad_pp is True:
            gcode += ';NCCAD9 G-code generated by FlatCAM Plus v%s - Version Date:    %s\n' % \
                     (str(self.app.version), str(self.app.version_date)) + '\n'
            ...
            gcode += '%sG-code generated by FlatCAM Plus v%s - Version Date: %s%s\n' % \
            ...
            gcode += self.machine_profile_gcode_header(
                comment_start_symbol=start_comment,
                comment_stop_symbol=stop_comment
            )

        return gcode

    def active_machine_profile(self):
        try:
            ...
            self.app.log.debug("FlatCAMCNCJob.active_machine_profile() error: --> %s" % str(e))
            return None

    def machine_profile_gcode_header(self, style="paren", comment_start_symbol="(", comment_stop_symbol=")"):
        profile = self.active_machine_profile()
        if not profile:
            return ''

        try:
            travel_x = float(profile.get("travel_x", 0) or 0)
            travel_y = float(profile.get("travel_y", 0) or 0)
            travel_z = float(profile.get("travel_z", 0) or 0)
            safe_z = float(profile.get("safe_z", 0) or 0)
            spindle_max = int(float(profile.get("spindle_max", 0) or 0))
        except (TypeError, ValueError):
            return ''

        lines = [
            "Machine Profile: %s" % str(profile.get("name", "Machine")),
            "Machine Travel: X%.3f Y%.3f Z%.3f mm" % (travel_x, travel_y, travel_z),
            "Machine Safe Z: %.3f mm" % safe_z,
            "Machine Spindle Max: %d RPM" % spindle_max,
        ]
        ...
        start_comment = comment_start_symbol if comment_start_symbol is not None else '('
        stop_comment = comment_stop_symbol if comment_stop_symbol is not None else ')'
        return ''.join("%s%s%s\n" % (start_comment, line, stop_comment) for line in lines) + '\n'
